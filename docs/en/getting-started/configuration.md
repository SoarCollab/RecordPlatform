...
| `FISCO_SOLC_CACHE_DIR` | Explicit verified exact-compiler cache used by guarded deployment | Required for contract deployment |
| `FISCO_ECC_SOLC` / `FISCO_SM_SOLC` | Optional explicit compiler paths; each must resolve inside the verified cache release | Leave empty to resolve from the cache |
