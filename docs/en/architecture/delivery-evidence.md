# P0/P1/P2 Delivery Evidence

The P0-P2 implementation evidence on this page was frozen from the last pre-documentation exact-main baseline, `main@0e091261233e7430ce5b904ad84d33569de910a1`, on 2026-07-27. P2-5 is accepted separately only when its later merge commit passes exact-main checks and Pages deployment. This separation prevents a passing static check from being presented as a production benchmark or an unpublished page from being described as online.
...
| P2-2 bounded downloads | `NONE`, legacy v1, and framed AEAD v2 are explicit paths; supported browsers write through a transactional File System Access sink | `boundedDownloader.ts`, `downloadSink.ts`, `fileSize.ts`, framed v2 migration/fixtures | Chromium 64/256/512 MiB gates keep application buffers non-growing and abort tamper/truncation/cancel/sink failures before commit |
...
The docs workspace itself passes `pnpm audit --audit-level low` after P2-Q3. The default branch still has **26 open Dependabot alerts: 8 Medium and 18 Low**. They remain explicit P6 supply-chain work. This page does not claim repository-wide zero advisories and does not reinterpret a High/Critical-only scanner as evidence that Medium/Low alerts are absent.
