...
5. each contract has a complete transaction hash, block number, and actual activation time; empty or partial legacy evidence is rejected;
6. the active-chain client returns the same transaction hash, configured contract address, and block number from an explicitly successful deployment receipt (FISCO status `0`, Besu status `1`); and
7. the node's complete runtime-code fingerprint matches the signed runtime artifact for the actual chain/crypto variant, and `contractIdentity()` exactly matches the selected catalog name/version.

The resulting `record-platform-contract-registry-entry.v1` fingerprint binds chain type/ID/group, address, semantic version, ABI and creation-bytecode hashes, observed runtime-code hash, deployment evidence, status, effective time, and upgrade strategy. Catalog `effectiveAt` is the earliest artifact lifecycle time; the mandatory `FISCO_*_DEPLOYMENT_EFFECTIVE_AT` is the actual active-chain time recorded in the runtime entry. The receipt query uses the same already-verified active client whose chain/group identity was inspected, so a configured triplet cannot be validated against another chain namespace. Missing receipts, JSON-RPC errors, failed or unknown status, transaction/address/block mismatches, duplicate addresses, and duplicate deployment transactions prevent publication of the immutable registry map. `SharingService`, `StorageService`, and the Besu adapter only use the resolved registry address. Registry RPC reads and batch store/query RPCs remain protected by the backend-to-FISCO shared token.

The local FISCO deployment script resolves one interactive Console launcher before any Console command. It prefers the official Console 3.7 `start.sh`; only a layout without `start.sh` may use an executable regular non-symlink `console.sh` as a documented legacy fallback. Explicit CLI/environment overrides must agree, use an allow-listed basename, and resolve directly inside the Console directory. All multiline identity, receipt, code, and identity calls reuse that launcher. The script queries official Console `getGroupInfo` before compilation and before each chain write, requiring its unique chain/group/crypto/VM tuple to match `FISCO_CHAIN_ID`/`FISCO_GROUP_ID` and an EVM target. VM metadata accepts boolean `isWasm` or legacy boolean `wasm`; if both exist they must be booleans with equal values. An explicit provisioner exports full source commit `6b4cc280eb884f1852e1ddf12aeea56a2103148c` with the upstream release markers, serially builds ECC/GM, and atomically publishes only binaries whose platform, mode, exact version and SHA-256 match the reviewed manifest. Ordinary deployment never downloads/builds compiler code and rejects Console's shared cache. It directly compiles with EVM London, optimizer disabled, and IPFS metadata, requires every ABI/creation/runtime output to match the signed artifacts, and atomically stages only verified ABI/creation bytes into Console immediately before a write. After each deploy command, it executes `getGroupInfo` and `getTransactionReceipt` in one Console session, accepts exactly one structured receipt with explicit status `0`, and cross-checks its transaction hash and contract address against the unique deploy output; the final tx/address/block fields all come from that receipt. It then checks the complete `getCode` bytes for the node's actual crypto variant before catalog-derived `contractIdentity()`. After successful verification it generates one UTC effective time, atomically publishes a credential-free `record-platform-contract-deployment-receipt.v2` receipt with `chainType=LOCAL_FISCO` and per-contract `receiptStatus=SUCCESS`, and only then atomically writes both addresses and complete deployment triplets to `.env`. Historical `v1` files remain audit records but do not bypass startup RPC verification. Invalid launcher/toolchain selection, provenance/version/path drift, missing/conflicting/wrong-type VM metadata, WASM, receipt absence/failure/ambiguity, parsing failure, field mismatch, chain/group/crypto/VM mismatch, runtime or identity mismatch, or revert prevents activation. Dry-run performs no Console call and writes no receipt or environment file.

Verified creation submission uses the project-packaged SDK 3.8.0 helper, never Console `deploy Name` or Console SDK jar discovery. Python captures bounded catalog/raw ABI/creation bytes through no-follow descriptors and passes a private length-framed stdin snapshot; Java validates those same bytes and submits the exact creation array through the raw SDK API. `FISCO_PRIVATE_KEY` is mandatory and shared with local provider writes to preserve `Sharing.operator`. Same-client chain/group/crypto/EVM checks precede signing. Helper timeouts are uncertain and never retried automatically; independent Console receipts, full runtime equality and audit-before-env remain required. See [deployment setup](../deployment/environment-setup.md) for build, signer, IPC and recovery details.
...
2. Explicitly provision and offline-verify the reviewed compiler cache with `provision_fisco_solc.py`, run `contract_fingerprint.py verify`, then `scripts/contract-deploy.sh`. The script validates toolchain provenance, chain/group/crypto/VM and complete compiled artifacts before deployment, binds each deployment to one explicitly successful receipt, verifies complete runtime code plus exact catalog identity afterwards, publishes the structured deployment receipt, and atomically activates both addresses plus transaction/block/effective-time evidence.
...
For application rollback, first disable production batch creation, restore the previous reviewed catalog/artifact set, its two addresses, and both complete transaction/block/effective-time triplets as one unit, then restart `platform-fisco`. Startup must re-read both successful receipts from the currently configured chain before the rollback is accepted. If either historical receipt cannot be proved, keep the service stopped and redeploy; never fabricate a triplet or weaken the startup gate. Retain every deployment receipt, migration `V1.13.0`, and all stored snapshots: dropping columns, deleting old catalog entries/receipts, or rewriting snapshots would destroy audit evidence.
