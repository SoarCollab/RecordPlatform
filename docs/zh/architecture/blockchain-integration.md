...
应用回滚时，先关闭生产批次创建，把上一版本已审查的 catalog/制品集合、两个地址和两组完整交易/区块/生效时间三元组作为一个整体恢复，再重启 `platform-fisco`。必须保留全部部署回执、`V1.13.0` 迁移和全部快照；删除列、删除旧 catalog 条目/回执或改写快照都会破坏审计证据。
