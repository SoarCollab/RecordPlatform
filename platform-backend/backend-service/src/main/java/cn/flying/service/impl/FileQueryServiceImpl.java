...
public class FileQueryServiceImpl implements FileQueryService {
    ...
    public FileDownloadMetadataVO getDownloadMetadata(Long userId,
        ...
        Long shareId = accessContext.friendShare() != null
                ? accessContext.friendShare().getId()
                : null;
        DownloadAccessBinding accessBinding = new DownloadAccessBinding(
                accessKind.name(),
                file.getTenantId() != null ? file.getTenantId() : TenantContext.getTenantIdOrDefault(),
                userId,
                shareId,
                null);
        return FileDownloadMetadataBuilder.build(
                ...
                fileHash,
                decryptInfo,
                () -> buildFileDecryptInfo(
                        accessContext, fileHash, userId,
                        keyDeliveryProtocol, downloadSessionId, true),
                accessBinding,
