...
HAVING 
    COUNT(*) > 100 
    AND TIMESTAMPDIFF(SECOND, MIN(operation_time), MAX(operation_time)) < 300 -- 5分钟内
ORDER BY 
...
BEGIN
    ...
    IF v_has_anomalies THEN
        INSERT INTO sys_operation_log (
            ...
            '安全预警', 
            v_anomaly_details, 
            'proc_check_operation_anomalies', 
            '/system/audit/anomalies', 
            'SYSTEM', 
            '127.0.0.1', 
            '系统自动检查', 
            0, 
...
BEGIN
    ...
    -- 创建备份表
    SET v_sql = CONCAT(
        'CREATE TABLE ', v_backup_table_name, ' LIKE sys_operation_log'
    );
