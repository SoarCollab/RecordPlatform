...
(
    ...
    origin         bigint        null comment '来源文件Id(标识分享文件原始来源)',
    file_name      varchar(255)  not null comment 'fileName',
    classification varchar(50)   null comment '文件分类',
    file_param     varchar(255)  null comment '文件参数(JSON):文件类型、文件描述、文件大小等其它参数',
    file_hash      varchar(255)  null comment 'fileHash',
    status         int           not null comment '文件上传状态(是否成功上传至区块链与分布式存储): 见——>FileUploadStatus',
    deleted        int default 0 not null comment 'deleted',
    create_time    datetime      not null comment 'createTime',
    contract_hash  varchar(255)  null comment '交易hash',
