...
public interface AuditLogMapper extends BaseMapper<AuditLog> {
     ...
     * @return 操作日志列表
     */
    @Select("SELECT * FROM audit_log WHERE operation_type = #{operationType} " +
            "AND operation_time BETWEEN #{startTime} AND #{endTime} " +
            "ORDER BY operation_time DESC")
     ...
     * @return 操作日志列表
     */
    @Select("SELECT * FROM audit_log WHERE module = #{module} " +
            "AND operation_time BETWEEN #{startTime} AND #{endTime} " +
            "ORDER BY operation_time DESC")
    List<AuditLog> findByModuleAndTimeRange(@Param("module") String module,
                                            @Param("startTime") LocalDateTime startTime,
                                            @Param("endTime") LocalDateTime endTime);
     ...
     * @return 操作日志列表
     */
    @Select("SELECT * FROM audit_log WHERE client_ip = #{clientIp} " +
            "AND operation_time BETWEEN #{startTime} AND #{endTime} " +
            "ORDER BY operation_time DESC")
    List<AuditLog> findByClientIpAndTimeRange(@Param("clientIp") String clientIp,
                                              @Param("startTime") LocalDateTime startTime,
                                              @Param("endTime") LocalDateTime endTime);
     ...
     * @return 失败操作日志列表
     */
    @Select("SELECT * FROM audit_log WHERE operation_status = 0 " +
            "AND operation_time BETWEEN #{startTime} AND #{endTime} " +
            "ORDER BY operation_time DESC")
     ...
     * @return 操作日志列表
     */
    @Select("SELECT * FROM audit_log WHERE risk_level = #{riskLevel} " +
            "AND operation_time BETWEEN #{startTime} AND #{endTime} " +
            "ORDER BY operation_time DESC")
    List<AuditLog> findByRiskLevelAndTimeRange(@Param("riskLevel") String riskLevel,
                                               @Param("startTime") LocalDateTime startTime,
                                               @Param("endTime") LocalDateTime endTime);
     ...
     * @return 删除的记录数
     */
    @Select("DELETE FROM audit_log WHERE operation_time < #{expireTime}")
    ...
    @Select("SELECT * FROM audit_log " +
            ...
            "AND (risk_level IN ('HIGH', 'CRITICAL') OR operation_status = 0) " +
            "AND operation_time BETWEEN #{startTime} AND #{endTime} " +
            "ORDER BY operation_time DESC")
    List<AuditLog> findAbnormalLogins(@Param("startTime") LocalDateTime startTime,
                                      @Param("endTime") LocalDateTime endTime);
