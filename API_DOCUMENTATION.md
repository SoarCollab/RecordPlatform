...
## 18. Integrity Alerts Module (Admin)

Base Path: `/api/v1/admin/integrity-alerts`

Storage integrity alerts are generated by the periodic integrity check that verifies S3 file existence and DB-to-chain hash consistency.

### 18.1 List Integrity Alerts

Query integrity alerts with pagination and optional filtering.
...
| alertType | string | No       | Filter by alert type: `S3_MISSING`, `HASH_MISMATCH` |

---

### 18.2 Trigger Manual Integrity Check

Manually trigger an integrity check outside the scheduled window.

```
POST /api/v1/admin/integrity-alerts/check
```
...
### 18.3 Acknowledge an Alert

Mark an alert as acknowledged by an admin.

```
PUT /api/v1/admin/integrity-alerts/{id}/acknowledge
```
...
### 18.4 Resolve an Alert

Resolve an alert with an optional resolution note.

```
PUT /api/v1/admin/integrity-alerts/{id}/resolve
```
