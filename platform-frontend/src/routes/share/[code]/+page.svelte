<script lang="ts">
	...
	async function loadSharedFiles() {
		...
		try {
			...
			error = err instanceof Error ? err.message : '分享链接无效或已过期';
		} finally {
			loading = false;
		}
	}
	...
	async function handleSaveSelected() {
		...
		try {
			...
			notifications.success('保存成功', `已将 ${selectedFiles.size} 个文件保存到您的账户`);
			selectedFiles = new Set();
		} catch (err) {
				...
				<Card.Root class="mt-6">
					<Card.Content class="flex items-center justify-between p-6">
						<div>
							...
							<p class="text-sm text-muted-foreground">将文件保存到您的账户，方便日后管理</p>
						</div>
