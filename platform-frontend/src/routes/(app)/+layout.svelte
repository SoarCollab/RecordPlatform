<script lang="ts">
	...
	function handleSSEMessage(message: SSEMessage) {
		switch (message.type) {
			...
			case 'ticket-updated': {
				...
				notifications.info('工单更新', data.title ? `工单「${data.title}」状态已更新` : '您的工单有新回复');
				break;
			}
			case 'file-processed': {
				...
				if (data.status === 'completed') {
					notifications.success('文件处理完成', data.fileName || '您的文件已处理完毕');
				} else if (data.status === 'failed') {
					notifications.error('文件处理失败', data.fileName || '文件处理过程中出错');
				}
				break;
			}
			case 'badge-update': {
				// Direct badge count update from server
				const data = message.data as { messages?: number; announcements?: number; tickets?: number };
				if (typeof data.messages === 'number') badges.updateMessageCount(data.messages);
				if (typeof data.announcements === 'number') badges.updateAnnouncementCount(data.announcements);
				if (typeof data.tickets === 'number') badges.updateTicketCount(data.tickets);
				break;
			}
			case 'notification': {
				// Generic notification
				const data = message.data as { title?: string; message?: string; type?: string };
	...
	const menuItems = [
		...
		{ href: '/tickets', icon: 'ticket', label: '工单系统', badgeKey: 'tickets' as const },
	...
	function getBadgeCount(key: 'messages' | 'announcements' | 'tickets' | null): number {
		if (!key) return 0;
		switch (key) {
			case 'messages':
...
<div class="flex h-screen">
	...
	>
		...
		<div class="flex h-16 items-center border-b px-4">
			...
			{:else}
				<div class="mx-auto flex h-8 w-8 items-center justify-center rounded-lg bg-primary text-primary-foreground">
					<svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
						<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" />
					</svg>
				</div>
		...
		<nav class="flex-1 overflow-y-auto p-2">
					...
					<li>
						...
						>
							...
							{#if !sidebarCollapsed}
								...
								{#if badgeCount > 0}
									<span class="flex h-5 min-w-5 items-center justify-center rounded-full bg-destructive px-1.5 text-xs font-medium text-white">
			...
			{#if auth.isAdmin}
				...
				<p class="mb-2 px-3 text-xs font-medium uppercase text-muted-foreground" class:hidden={sidebarCollapsed}>
					管理功能
				</p>
				<ul class="space-y-1">
		...
		<div class="border-t p-2">
			...
			>
				<svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" class:rotate-180={sidebarCollapsed}>
		...
		<header class="flex h-16 items-center justify-between border-b bg-card px-6">
			...
			<div class="flex items-center gap-4">
				...
				<button
					...
					onclick={toggleMode}
					aria-label={mode.current === 'dark' ? '切换到亮色模式' : '切换到暗色模式'}
					title={mode.current === 'dark' ? '切换到亮色模式' : '切换到暗色模式'}
				>
				...
				<!-- SSE Status -->
				<div class="flex items-center gap-2 text-sm text-muted-foreground" title={`SSE: ${sse.status}`}>
					<span
					...
					{#if !sidebarCollapsed}
						<span class="hidden sm:inline">
							{sse.isConnected ? '已连接' : sse.status === 'connecting' ? '连接中' : '未连接'}
						</span>
					{/if}
				</div>
