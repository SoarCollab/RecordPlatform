<script lang="ts">
  ...
  function handleSSEMessage(message: SSEMessage) {
    ...
    if (
      ...
      msgData.conversationId !== data.conversationId
    )
      return;
  ...
  async function loadConversationAndMessages() {
    ...
    try {
      const detail: ConversationDetailVO = await getConversationDetail(
        data.conversationId,
        {
          pageNum: 1,
          pageSize,
      ...
      total = toNumber(
        ...
        toNumber(detail.messages?.total, 0),
      );
      page = 1;
  ...
  async function loadMoreMessages() {
    ...
    try {
      ...
      messages = [
        ...(detail.messages?.records ?? []).slice().reverse(),
        ...messages,
      ];
      page = nextPage;
      hasMore = Boolean(detail.hasMore);
...
<svelte:head>
  <title
    >{conversation ? `与 ${conversation.otherUsername} 的对话` : "消息"} - 存证平台</title
  >
</svelte:head>
  ...
  <div class="flex items-center gap-4 border-b pb-4">
    ...
    {#if conversation}
      <div class="flex items-center gap-3">
        <div
          class="bg-primary/10 text-primary flex h-10 w-10 items-center justify-center rounded-full text-lg font-medium"
        >
        ...
        <div class="space-y-4">
          {#each messages as msg (msg.id)}
            ...
            <div class="flex {isOwn ? 'justify-end' : 'justify-start'}">
              <div
                class="flex max-w-[70%] gap-2 {isOwn ? 'flex-row-reverse' : ''}"
              >
                <div
                ...
                <div>
                  ...
                  <p
                    class="text-muted-foreground mt-1 text-xs {isOwn
                      ? 'text-right'
                      : ''}"
                  >
