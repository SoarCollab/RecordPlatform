<script lang="ts">
	...
	const canPreview = $derived(
		file && isPreviewable(file.contentType)
	);
	...
	async function handlePreview() {
		...
		try {
			...
			notifications.error('加载预览失败', err instanceof Error ? err.message : '请稍后重试');
		} finally {
	...
	{:else if file}
			...
			<Card.Content class="space-y-4">
				...
				{#if file.status === FileStatus.COMPLETED}
					...
					<div class="flex flex-wrap gap-2">
						...
						<Button variant="outline" onclick={() => { shareCode = ''; shareDialogOpen = true; }}>
							<svg class="mr-2 h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
				...
				<Card.Content class="space-y-4">
					<div class="rounded-lg bg-muted/50 p-4">
						<div class="grid gap-4">
							<div>
								<p class="text-sm font-medium text-muted-foreground">交易哈希</p>
								<p class="mt-1 break-all font-mono text-sm">{file.transactionHash}</p>
							</div>
							{#if transaction}
								<div class="grid gap-4 sm:grid-cols-2">
									<div>
										<p class="text-sm font-medium text-muted-foreground">区块号</p>
										<p class="mt-1 font-mono text-sm">{transaction.blockNumber}</p>
									</div>
									<div>
										<p class="text-sm font-medium text-muted-foreground">存证时间</p>
										<p class="mt-1 text-sm">{formatDateTime(transaction.timestamp)}</p>
									</div>
								</div>
		...
		{#if showPreview && previewUrl && file}
			<Card.Root>
				...
				<Card.Content>
					<FilePreview url={previewUrl} contentType={file.contentType} fileName={file.fileName} />
				</Card.Content>
			</Card.Root>
		{/if}
	{/if}
