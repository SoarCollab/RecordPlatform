<script lang="ts">
	...
	async function handleChangePassword() {
		...
		try {
			...
			notifications.error('修改失败', err instanceof Error ? err.message : '请检查原密码是否正确');
		} finally {
...
<div class="mx-auto max-w-2xl space-y-6">
	...
	<div class="rounded-lg border bg-card">
		...
		<div class="space-y-4 p-4">
			<div>
				<label for="settings-username" class="mb-2 block text-sm font-medium">用户名</label>
				<input
			...
			<div>
				<label for="settings-nickname" class="mb-2 block text-sm font-medium">昵称</label>
				<input
			...
			<div>
				<label for="settings-email" class="mb-2 block text-sm font-medium">邮箱</label>
				<input
					id="settings-email"
					type="email"
					bind:value={email}
			...
			<div>
				<label for="settings-phone" class="mb-2 block text-sm font-medium">手机号</label>
				<input
					id="settings-phone"
					type="tel"
					bind:value={phone}
	...
	<div class="rounded-lg border bg-card">
		...
		<div class="space-y-4 p-4">
			<div>
				<label for="settings-old-password" class="mb-2 block text-sm font-medium">原密码</label>
				<input
			...
			<div>
				<label for="settings-new-password" class="mb-2 block text-sm font-medium">新密码</label>
				<input
			...
			<div>
				<label for="settings-confirm-password" class="mb-2 block text-sm font-medium">确认新密码</label>
				<input
	...
	<div class="rounded-lg border bg-card">
		...
		<div class="space-y-3 p-4 text-sm">
			<div class="flex justify-between">
				...
				<span class="font-medium">{auth.user?.role === 'admin' ? '管理员' : '普通用户'}</span>
			</div>
			<div class="flex justify-between">
				<span class="text-muted-foreground">注册时间</span>
				<span>{auth.user?.createTime || '-'}</span>
			</div>
			<div class="flex justify-between">
