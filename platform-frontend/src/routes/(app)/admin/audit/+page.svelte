<script lang="ts">
  ...
  /** 从仪表盘钻取传递的筛选条件 */
  let drillDownFilters = $state<DrillDownFilter | null>(null);
  let drillDownCounter = $state(0);
  ...
  /** 处理仪表盘钻取 */
  function handleDrillDown(filters: DrillDownFilter) {
    drillDownFilters = { ...filters };
    drillDownCounter++;
    activeTab = "logs";
  }

  /** 包含 counter 的 filter 对象，确保每次钻取都能触发 AuditLogList 的 $effect */
  const externalFilters = $derived(
    drillDownFilters
      ? { ...drillDownFilters, _counter: drillDownCounter }
      : null,
  );
...
<div class="space-y-6">
  <div class="flex items-center justify-between">
    <div>
      ...
      <p class="text-muted-foreground">操作日志监控与审计配置</p>
    </div>
  ...
  <Tabs.Root value={activeTab} onValueChange={(v) => (activeTab = v)}>
    <Tabs.List>
      ...
      <Tabs.Trigger value="logs">操作日志</Tabs.Trigger>
    </Tabs.List>

    <Tabs.Content value="dashboard" class="mt-4">
      <AuditDashboard onDrillDown={handleDrillDown} />
    </Tabs.Content>

    <Tabs.Content value="logs" class="mt-4">
      <AuditLogList {externalFilters} />
    </Tabs.Content>
  </Tabs.Root>
</div>
