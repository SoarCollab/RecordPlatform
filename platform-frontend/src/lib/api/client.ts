...
import { type Result, ResultCode, UNAUTHORIZED_CODES, RETRYABLE_CODES } from './types';

// ===== Configuration =====

const API_BASE = '/record-platform/api/v1';
const MAX_RETRIES = 3;
...
export function setToken(token: string, expire: string, rememberMe: boolean = true): void {
	if (!browser) return;
...
export class ApiError extends Error {
	...
	constructor(code: number, message: string) {
		...
		this.name = 'ApiError';
		this.code = code;
		this.isUnauthorized = (UNAUTHORIZED_CODES as readonly number[]).includes(code);
