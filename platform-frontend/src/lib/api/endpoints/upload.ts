...
import type { StartUploadVO, StartUploadRequest, ProgressVO, FileUploadStatusVO } from '../types';
...
export async function resumeUpload(clientId: string): Promise<StartUploadVO> {
	return api.post<StartUploadVO>(`${BASE}/resume`, null, { params: { clientId } });
}
 ...
 */
export async function checkUploadStatus(clientId: string): Promise<FileUploadStatusVO | null> {
