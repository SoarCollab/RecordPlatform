 ...
 * 必填验证
 */
export function required(value: unknown, fieldName = '此字段'): ValidationResult {
 ...
 * 最小长度验证
 */
export function minLength(value: string, min: number, fieldName = '此字段'): ValidationResult {
	if (value.length < min) {
 ...
 * 最大长度验证
 */
export function maxLength(value: string, max: number, fieldName = '此字段'): ValidationResult {
	if (value.length > max) {
...
export function username(value: string): ValidationResult {
	...
	if (!usernameRegex.test(value)) {
		return { valid: false, message: '用户名只能包含字母、数字和下划线，长度4-20' };
	}
	return { valid: true };
}
 ...
 * 确认密码验证
 */
export function confirmPassword(value: string, original: string): ValidationResult {
	if (value !== original) {
 ...
 * 文件类型验证
 */
export function fileType(filename: string, allowedTypes: string[]): ValidationResult {
	const ext = filename.split('.').pop()?.toLowerCase() || '';
	if (!allowedTypes.includes(ext)) {
		return { valid: false, message: `只支持以下格式：${allowedTypes.join(', ')}` };
	}
	return { valid: true };
}
