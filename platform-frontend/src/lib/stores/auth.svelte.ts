...
import type { AccountVO, LoginRequest, RegisterRequest, UpdateUserRequest } from '$api/types';
...
async function login(credentials: LoginRequest, options: LoginOptions = {}): Promise<void> {
...
async function register(data: RegisterRequest, options: LoginOptions = {}): Promise<void> {
	const { rememberMe = true } = options;
	isLoading = true;
	error = null;

	try {
		const result = await authApi.register(data, rememberMe);
		user = {
			id: '',
			username: result.username,
			role: result.role,
