...
function calculateChunks(fileSize: number, chunkSize: number = DEFAULT_CHUNK_SIZE): number {
...
async function startUpload(id: string): Promise<void> {
	...
	try {
		...
		updateTask(id, {
			...
			progress: Math.round((result.processedChunks.length / task.totalChunks) * 100)
		});
