...
基于 FISCO BCOS 区块链和 S3 兼容存储 分布式存储的企业级文件存证平台。
...
- 多租户隔离：数据库/Redis/S3 兼容存储 路径租户隔离
- 审计追踪：完整的操作日志和审计记录
...
2. **chunk**: 分片上传，每片使用配置的算法（AES-GCM/ChaCha20）加密后双写 S3 兼容存储
...
| 存储       | S3 兼容存储, MySQL, Redis    | 8.5.9, 8.0+, 6.0+ |
...
# 生产环境启动（带 SkyWalking 链路追踪）
./scripts/start-all-skywalking.sh prod

# 单独启动某个服务（带 SkyWalking）
...
> **启动顺序**：Storage → Fisco → Backend Web（因 Dubbo RPC 依赖关系）

Profile: `local`, `dev`, `prod`

详细部署配置请参阅 [scripts/README.md](./scripts/README.md)。
...
│   ├── start-all-skywalking.sh   # 生产环境一键启动（带 SkyWalking）
...
| 端点                        | 说明                                                            |
| --------------------------- | --------------------------------------------------------------- |
| `/actuator/health`          | 健康状态 (DB/Redis/RabbitMQ/S3 兼容存储/Saga/Outbox/Encryption) |
